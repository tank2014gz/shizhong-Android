...
//			R.drawable.ease_chat_location_selector };
	...
	private String exifStr = "{\"headerUrl\":\"%s\",\"nickname\":\"%s\",\"memberId\":\"%s\"}";
...
//		nickName = fragmentArgs.getString(EaseConstant.EXTRA_CHAT_NICKNAME);
		nickName= PrefUtils.getString(getActivity(),ContantsActivity.LoginModle.NICK_NAME,"");
		headUrl = fragmentArgs.getString(EaseConstant.EXTRA_CHAT_HEADURL);
//		headUrl=PrefUtils.getString(getActivity(),ContantsActivity.LoginModle.HEAD_URL,"");
		mMineHeadUrl = fragmentArgs.getString(ContantsActivity.LoginModle.HEAD_URL);
		mMineUserId=PrefUtils.getString(getActivity(),ContantsActivity.LoginModle.LOGIN_USER_ID,"");
	...
	protected void onConversationInit() {
		...
		try {
			...
			extfile = conversation.getExtField();
			if (TextUtils.isEmpty(extfile)) {
				extfile = String.format(this.exifStr, mMineHeadUrl, nickName, mMineUserId);

				conversation.setExtField(extfile);
			}

			JSONObject json = new JSONObject(extfile);
	...
	protected void onMessageListInit() {
		messageList.init(toChatUsername, chatType,
				chatFragmentHelper != null ? chatFragmentHelper.onSetCustomChatRowProvider() : null, conversation,
				mMineHeadUrl);
	...
	protected void sendTextMessage(String content) {
		...
		event.put("chat_id", conversation.getUserName());
		event.put("type", "聊天");
		event.put("time", DateUtils.format9(System.currentTimeMillis()));
		MobclickAgent.onEvent(getActivity(), "chat_ID", event);
		...
		try {
			...
			if (!TextUtils.isEmpty(str)) {
				...
				// {"headerUrl":"http://7xpj9r.com1.z0.glb.clouddn.com/boy1457250350614020.jpg?imageView2/0/w/540/h/540","nickname":"电车上的痴汉","memberId":"96a0a6437b514d78b6632a177c50623b"}
				String nickName = msgJSON.getString("nickname");
				// String headUrl = msgJSON.getString("headerUrl");
				message.setAttribute("nickName", nickName);
				// message.setAttribute("headerUrl", headUrl);
			} else {
				JSONObject msgJSON = new JSONObject(extfile);
				// {"headerUrl":"http://7xpj9r.com1.z0.glb.clouddn.com/boy1457250350614020.jpg?imageView2/0/w/540/h/540","nickname":"电车上的痴汉","memberId":"96a0a6437b514d78b6632a177c50623b"}
				String nickName = msgJSON.getString("nickname");
				// String headUrl = msgJSON.getString("headerUrl");
				message.setAttribute("nickName", nickName);

...
public class EaseNotifier {
	...
	protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
		...
		System.out.println("nickn" + nickName);
		...
		try {
			...
			if (notificationInfoProvider != null) {
				...
				if (customNotifyText != null) {
					...
					// String msg =
					//
					// EMClient.getInstance().chatManager().getConversation(username).getExtField();
					// try {
					// JSONObject msgJson = new JSONObject(msg);
					// notifyText = msgJson.getString("nickname");
					// } catch (JSONException e1) {
					// // TODO Auto-generated catch block
					// e1.printStackTrace();
					// }
					// 4d97f1e75ce14737a9e699b0aea0fc31: 头像
					customNotifyText = customNotifyText.substring(customNotifyText.indexOf(":"));
					if (nickName == null) {
						notifyText = "您收到一条消息";
					} else {
						notifyText = nickName + customNotifyText;
					}
			...
			if (notificationInfoProvider != null) {
				// lastest text
				String customSummaryBody = notificationInfoProvider.getLatestText(message, fromUsersNum,
						notificationNum);

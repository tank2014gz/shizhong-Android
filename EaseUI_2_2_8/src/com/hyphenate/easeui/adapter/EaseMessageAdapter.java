...
import com.hyphenate.easeui.widget.chatrow.EaseChatRowRevoke;
...
public class EaseMessageAdapter extends BaseAdapter {
	...
	public EaseMessageAdapter(Context context, String username, int chatType, ListView listView,
			EMConversation convsation, String fromHeader) {
		this.context = context;
		this.listView = listView;
		toChatUsername = username;
		this.fromHeader = fromHeader;
		this.conversation = convsation;
	}
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
		...
		case TXT:
			if (message.getBooleanAttribute(EaseConstant.EASE_ATTR_REVOKE, false)) {
				chatRow = new EaseChatRowRevoke(context, message, position, this, conversation, this.fromHeader);
			} else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
				chatRow = new EaseChatRowBigExpression(context, message, position, this, conversation, this.fromHeader);
			} else {
				chatRow = new EaseChatRowText(context, message, position, this, conversation, this.fromHeader);
			}
			break;
		case LOCATION:
			chatRow = new EaseChatRowLocation(context, message, position, this, conversation, this.fromHeader);
			break;
		case FILE:
			chatRow = new EaseChatRowFile(context, message, position, this, conversation, this.fromHeader);
			break;
		case IMAGE:
			chatRow = new EaseChatRowImage(context, message, position, this, conversation, this.fromHeader);
			break;
		case VOICE:
			chatRow = new EaseChatRowVoice(context, message, position, this, conversation, this.fromHeader);
			break;
		case VIDEO:
			chatRow = new EaseChatRowVideo(context, message, position, this, conversation, this.fromHeader);
			break;
		default:
			break;
		}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		// 0if(message.getBooleanAttribute(EaseConstant.EASE_ATTR_REVOKE,
		// false)){
		// ((EaseChatRowRevoke)convertView).setUpView(message, position,
		// itemClickListener);
		// }else{

...
import com.shizhong.view.ui.base.view.HorizontalScrollViewEx.OnFlingListener;

public class VideoSelectionView extends RelativeLayout implements OnFlingListener, OnCheckedChangeListener {
	/** 背景图片滚动区域 */
	...
	private RadioButton mCheckRectangle, mCheckSquare;
	...
	/** 边距信息设置 */
	private LayoutParams mSeekLeftLayoutParams, mSeekRightLayoutParams;
	/** 处理左右滑动 */
	...
	// private final int ACTION_CUT_VIDEO_IMAGE = 3;
	...
	private void init() {
		mPrepared = false;
		mThumbWidth = UIUtils.dipToPx(getContext(), 56);
		mSpace = UIUtils.dipToPx(getContext(), 20);
		mSeekMargin = UIUtils.dipToPx(getContext(), 3);
		...
		mRadioGroupBackground.setOnCheckedChangeListener(this);
		mCheckRectangle = (RadioButton) findViewById(R.id.change_rectangle);
		mCheckSquare = (RadioButton) findViewById(R.id.change_square);

		// videoModeControllerLayout =
		// findViewById(R.id.video_mode_controller_layout);
		...
		mSeekLeftLayoutParams = (LayoutParams) mSeekLeft.getLayoutParams();
		mSeekRightLayoutParams = (LayoutParams) mSeekRight.getLayoutParams();
		getCurruntImageThread.start();
		new Handler().postDelayed(new Runnable() {

			@Override
			public void run() {
				// TODO Auto-generated method stub
				mCursorBarWidth = mSeekLeft.getWidth();
			}
		}, 500);
	}

	public void setRectangleBackground(int backId) {
		if (mCheckRectangle != null) {
			mCheckRectangle.setBackgroundResource(backId);
		}
	}

	public void setDefaultModel() {
		mRadioGroupBackground.check(R.id.change_rectangle);
	}

	public void setChangeBackGroundVisiblty(boolean flag) {
		if (flag) {
	...
	public String getVideoCutTime() {
		...
		return gennerTimeSencond(time < 5 * 1000 ? 5 * 1000 : time);
	}
	...
	public void init(final File mThumbCacheDir, String videoPath, int duration, int maxDuration, int minDuration) {
		...
		int pad = UIUtils.dipToPx(mContext, 18);
		mWindowWidth = UIUtils.getScreenWidthPixels(getContext()) - pad * 2;
	...
	private void set60sVideo(int maxDuration, File mThumbCacheDir) {
		...
		int top = UIUtils.dipToPx(mContext, 18);
	...
	private void prepareThumbs(File mThumbCacheDir, final String videoPath) {
		...
		do {
			...
			String thumbPath = FileUtils.concatPath(thubmDir.getPath(), index + ".jpg");
		...
		if (thubmDir.exists() && thubmDir.canRead() && thubmDir.isDirectory() && thubmDir.list() != null
				&& thubmDir.list().length > 0) {
			// 不需要截图
			checkLoadThumb();
			return;
		}
	...
	private Thread getCurruntImageThread = new Thread() {
		@Override
		public void run() {
			while (!isRelease) {
				if (mThumbLoading) {
					File f = new File((mThubmDir.getPath() + File.separator), String.format("%d.jpg", thumbsIndex));
	...
	private boolean loadThumb(int startIndex, int startTime) {
		if (thumbs == null || TextUtils.isEmpty(videoPath) || mThumbLoading)
			return false;
		new AsyncTask<Void, Void, Boolean>() {
			...
			protected Boolean doInBackground(Void... params) {
				...
				thumbsIndex = 1;
				mThumbLoading = true;
				int total = thumbs.size();
				int during = VideoUtils.getVideoDuring(videoPath);
				String filePath = mThubmDir.getAbsolutePath();
				float timeUnit = ((float) during / total);
				for (int i = 1; i <= total; i++) {
					String file = filePath.concat(File.separator).concat(i + "").concat(".jpg");
					new File(file).deleteOnExit();
					float time = (timeUnit * (i - 1));
					Bitmap bitmap = VideoUtils.getFramBitamp(videoPath, time);
	...
	private void loadVideoThumbImageView(ArrayList<Thumb> thumbs, String videoPath) {
		if (thumbs != null && thumbs.size() > 0 && !TextUtils.isEmpty(videoPath)) {
			...
			for (Thumb thumb : thumbs) {
				VideoThumbImageView view = new VideoThumbImageView(getContext(), thumb.thumbPath, mWindowWidth,
						thumb.index, thumb.startTime);
	...
	private void onLeftToRight(int distance) {
		if (mLeftFocus) {
			...
			if (mSeekRightLayoutParams != null
					&& mSeekRightLayoutParams.rightMargin > mVideoSelection.getMinRightMargin()) {
	...
	protected boolean updateTimes() {
		if (mVideoSelection.getCurrentWidth() > 0 && mPrepared) {
			...
			int leftMargin = mVideoSelection.getLeftMargin();
			int scrollX = mVideoThumbnailsBackground.getScrollX();
			int start = (int) ((leftMargin + scrollX) * 1000F / mSecondWidth);
			int start_1 = (int) ((leftMargin + scrollX - (mCursorBarWidth / 2)) * 1000F / mSecondWidth);
			int end = start_1 + (int) ((mVideoSelection.getCurrentWidth()) * 1000.0F / mSecondWidth);
	...
	protected void refreshTimes() {
		...
		// mEndTime=62015
		mSelectionTimeView.setText(String.format("%s", gennerTime(total)));
	...
	// public void setOnBackgroundColorListener(OnBackgroundColorListener
	// onBackgroundColorListener) {
	...
	public void onCheckedChanged(RadioGroup group, int checkedId) {
		switch (checkedId) {
		case R.id.change_square:
			onVideoChangeScaleTypeListener.onChanged(FIT_XY);
			break;
		case R.id.change_rectangle:
	...
	public void release() {
		isRelease = true;
		if (getCurruntImageThread != null) {
			getCurruntImageThread = null;
		}
		if (thumbs != null) {
			thumbs.clear();
			thumbs = null;
		...
		if (onVideoChangeScaleTypeListener != null) {
			onVideoChangeScaleTypeListener = null;
		}
		if (onVideoChangeScaleTypeListener != null) {
			onVideoChangeScaleTypeListener = null;
		}
	...
	// if (thumbs == null || StringUtils.isEmpty(videoPath) || mThumbLoading ||
	// startEncoding)
	// return false;
	//
	...
	// int count = (startIndex + mPageSize) <= (total + 1) ? mPageSize : (total
	// - (startIndex - 1));
	...
	// // 快速截图方法：先切视频，后截图
	// cmd = String.format("ffmpeg %s -ss %.2f -i \"%s\" -t %.2f -vcodec copy
	// -acodec copy \"%s\"",
	// FFMpegUtils.getLogCommand(), startTime / 1000F, videoPath, (vframes + 1)
	// * r, tempVideo);
	...
	// mVideoRotation = ConvertToUtils.toInt(
	...
	// }
	//
	// cmd = String.format("ffmpeg %s -i \"%s\" -r 1/%.1f -start_number %d
	// -vframes %d -s %dx%d %s \"%s",
	// FFMpegUtils.getLogCommand(), tempVideo, r, startIndex, vframes,
	// mThumbWidth, mThumbWidth,
	// ffmpeg, mThubmDir.getPath()) + File.separator + "thumb_%d.jpg\"";
	//
	// Logger.e("cmd = " + cmd);
	//
	...
	// }
	//
	// File f = new File((mThubmDir.getPath() + File.separator),
	// String.format("thumb_%d.jpg", thumbsIndex));
	// if (f.exists() && f.canRead() && f.length() > 0) {
	// if (getContext() != null) {
	// Activity activity = (Activity) getContext();
	// if (activity != null && !activity.isFinishing()) {
	...
	// // FileUtils.deleteFile(tempVideo);
	// // // 快速截图方法：先切视频，后截图
	// // cmd = String.format("ffmpeg %s -ss %.2f -i \"%s\" -t %.2f -vcodec
	// // copy -acodec copy \"%s\"", FFMpegUtils.getLogCommand(), startTime /
	// // 1000F, videoPath, (vframes + 1) * r, tempVideo);
	// // if (UtilityAdapter.FFmpegRun("", cmd) != 0) {
	// // UtilityAdapter.stopEncodingLog(Logger.getIsDebug());
	// // CrashUncaughtException.sendFfmpegLog();
	// // return false;
	...
	// //
	// // cmd = String.format("ffmpeg %s -i \"%s\" -r 1/%.1f -start_number %d
	// // -vframes %d -s %dx%d %s \"%s", FFMpegUtils.getLogCommand(),
	// // tempVideo, r, startIndex, vframes, mThumbWidth, mThumbWidth, ffmpeg,
	// // mThubmDir.getPath()) + File.separator + "thumb_%d.jpg\"";
	// //
	...
	// // //快速截图方法：先切视频，后截图
	// // cmd =
	// // String.format("ffmpeg %s -ss %.2f -i \"%s\" -t %.2f -vcodec copy
	// // -acodec copy \"%s\"",
	...
	// // }
	// // cmd =
	// // String.format("ffmpeg %s -i \"%s\" -r 1/%.1f -start_number %d
	// // -vframes %d -s %dx%d \"%s",
	// // FFMpegUtils.getLogCommand(), tempVideo, r, startIndex, vframes,

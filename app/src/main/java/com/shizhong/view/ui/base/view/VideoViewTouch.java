...
public class VideoViewTouch extends TextureVideoView {
	...
	private static final String TAG=VideoViewTouch.class.getSimpleName();
	...
	protected void initVideoView() {
		super.initVideoView();
		mWindowWidth = UIUtils.getScreenWidthPixels(getContext());
	...
	public int getCropX() {
		if (mLayoutParams != null) {
			 return (int) (Math.abs(mLayoutParams.leftMargin) * 1.0F /
			 mLayoutParams.width * getVideoWidth());
//			return Math.abs(mLayoutParams.leftMargin);
	...
	public int getCropY() {
		if (mLayoutParams != null) {
			 return (int) (Math.abs(mLayoutParams.topMargin) * 1.0F /
			 mLayoutParams.height * getVideoHeight());
//			return Math.abs(mLayoutParams.topMargin);
	...
	public void resize() {
		...
		// Logger.e("[VideoViewTouch]videoWidth:" + videoWidth + " videoHeight:"
		// + videoHeight + " mWindowWidth:" + mWindowWidth);
		...
		// //支持判断手机屏幕宽高
		// if ((videoWidth >= videoHeight && videoWidth >= mWindowWidth) ||
		// (videoWidth <= videoHeight && videoHeight <= mWindowWidth)) {
		// mLayoutParams.height = mWindowWidth;
		// mLayoutParams.width = (int) (mWindowWidth * videoAspectRatio);
		// if (videoWidth > videoHeight)
		// setCanScrollX(true);
		...
		LogUtils.e(TAG,"[VideoEditActivity]videoWidth:" + videoWidth + "x" + videoHeight + " windowWidth:" + mWindowWidth
	...
	public void centerXY() {
		if (mLayoutParams != null) {
			if (mCanScrollX) {
				...
				mLayoutParams.rightMargin -= (mLayoutParams.width - mWindowWidth) / 2;
				setLayoutParams(mLayoutParams);
			} else if (mCanScrollY) {
				mLayoutParams.topMargin -= (mLayoutParams.height - mWindowWidth) / 2;
				mLayoutParams.bottomMargin -= (mLayoutParams.height - mWindowWidth) / 2;
				setLayoutParams(mLayoutParams);
			}
		}
	}
	...
	private void onRightToLeft(int distance) {
		// Logger.e("[VideoView]onRightToLeft leftMargin:" +
		// mLayoutParams.leftMargin + " width:" + mLayoutParams.width + "
	...
	private void onTopToBottom(int distance) {
		// Logger.e("[VideoView]onTopToBottom topMargin:" +
		// mLayoutParams.topMargin + " width:" + mLayoutParams.height + "
		// mWindowWidth:" + mWindowWidth + " distance:" + distance + "
		// mCanScrollY:" + mCanScrollY);
		if (mCanScrollY) {
	...
	private void onBottomToTop(int distance) {
		// Logger.e("[VideoView]onRightToLeft topMargin:" +
		// mLayoutParams.topMargin + " width:" + mLayoutParams.height + "
		// mWindowWidth:" + mWindowWidth + " distance:" + distance + "
		// mCanScrollY:" + mCanScrollY);
		if (mCanScrollY) {

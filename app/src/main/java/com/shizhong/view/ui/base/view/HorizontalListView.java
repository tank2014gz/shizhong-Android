...
//		int positionX = position * this.getWidth();
		int allWidth=this.getWidth();
		int account=this.getChildCount();
		int positionX=allWidth/account;
		int scrollLength=positionX*position;
//		int maxWidth = this.getChildCount() * this.getWidth();
		if (positionX <= 0) {
			positionX = 0;
		}
		if (positionX*position > allWidth) {
			scrollLength = allWidth;
		}
		scrollTo(scrollLength);
	}

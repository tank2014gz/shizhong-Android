...
public class GetuiSdkHttpPost {
    ...
    public static boolean isPostClientId;

    public  static  void  postClientId(final  Context context,String clientId,String token){

        if(TextUtils.isEmpty(clientId)||TextUtils.isEmpty(token)){
          return;
        }
        if(isPostClientId){
            return;
        ...
        String lastClientId=PrefUtils.getString(context,ContantsActivity.GeTui.CLIENT_ID,"");
        if(!TextUtils.isEmpty(lastClientId)&&clientId.equals(lastClientId)){
            return;
        }


        Map<String,String> params=new HashMap<>();
        params.put("getuiClientid",clientId);
        params.put("token",token);
        PrefUtils.putString(context, ContantsActivity.GeTui.CLIENT_ID,clientId);

        BaseHttpNetMananger.getInstance(context).postJSON(context, "/member/modifyGetuiClientid", params, new IRequestResult() {
            @Override
            ...
            public void requestFail() {
                LogUtils.e("shizhong ---","req:"  +context.getString(R.string.net_conected_error));
            }

            @Override
            public void requestNetExeption() {
                LogUtils.e("shizhong ---","req:"  +context.getString(R.string.net_error));
            }
        },false);
